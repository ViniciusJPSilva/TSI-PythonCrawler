...
def get_notice_data(notice_url: str) -> Notice:


    # Padroniza a URL.
    url = standardize_url(notice_url)
    ...
    title = soup.find(attrs={'class': 'documentFirstHeading'}).contents[0]
    modality = soup.find(attrs={'id': 'form-widgets-modalidade_edital'}).find(attrs={'class', 'selected-option'}).contents[0]
    number = int(soup.find(attrs={'id':'form-widgets-numero_edital'}).contents[0].replace('.',''))
    year = int(soup.find(attrs={'id':'form-widgets-ano_edital'}).contents[0].replace('.',''))
    situation = soup.find(attrs={'id': 'form-widgets-situacao_edital'}).find(attrs={'class', 'selected-option'}).contents[0]
    
    pdf_url = create_request(soup.find('table', class_='listing').find_all('a', 'contenttype-file')[0].get('href'))
    pdf_soup = BeautifulSoup(pdf_url.content, 'html.parser')
    pdf = pdf_soup.find(attrs={'id': 'content'}).find_all('a')[0].get('href')

    notice = Notice(title, modality, number, year, situation, pdf)
...
def create_links_list(url: str, classes: str, soup: BeautifulSoup) -> List[Tuple[str, str]]:
    ...
    """
    links = []
    for link_tag in soup.find('table', class_='listing').find_all('a', classes):
